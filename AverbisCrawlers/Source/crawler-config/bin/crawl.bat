...
if "%SEEDDIR%" == "" ( 
    echo Missing seedDir : crawl ^<seedDir^> ^<crawlDir^> ^<solrURL^> ^<numberOfRounds^> ^<outputPath^> ^(optional^)
    exit /B 1;
)

if "%CRAWL_PATH%" == "" (
    echo Missing crawlDir : crawl ^<seedDir^> ^<crawlDir^> ^<solrURL^> ^<numberOfRounds^> ^<outputPath^> ^(optional^)
    exit /B 1;
)

if "%SOLRURL%" == "" (
    echo Missing SOLRURL : crawl ^<seedDir^> ^<crawlDir^> ^<solrURL^> ^<numberOfRounds^> ^<outputPath^> ^(optional^)
    exit /B 1;
)

if "%LIMIT%" == "" (
    echo Missing numberOfRounds : crawl ^<seedDir^> ^<crawlDir^> ^<solrURL^> ^<numberOfRounds^> ^<outputPath^> ^(optional^)
    exit /B 1;
)
...
for /L %%i IN (1,1,%LIMIT%) do (
	...
	if defined OUTPUT_PATH set OUTPUT_DEF="-Deucases.servicewriter.output=%OUTPUT_PATH%"
	call %bin%nutch index "-Deucases.servicewriter.url=%SOLRURL%" "-Deucases.servicewriter.segments=%CRAWL_PATH%/segments" !OUTPUT_DEF! "%CRAWL_PATH%/crawldb" -linkdb "%CRAWL_PATH%/linkdb" "%CRAWL_PATH%/segments/!SEGMENT!"
	...
	SET NUTCH_OPTS=%NUTCH_OPTS_ENV%
	call %bin%nutch clean "-Deucases.servicewriter.url=%SOLRURL%" "-Deucases.servicewriter.segments=%CRAWLS_PATH%/segments" "%CRAWL_PATH%/crawldb"
